...
(function (global, factory) {
    ...
    RenderDom = {
        ...
        init :function(config){
            var content = '',
                date = config.currentDate ? config.currentDate : new Date();//当前渲染日期
            switch (config.renderType){//判断渲染类型
                case "day" :
        ...
        createDay : function(config,date){
            var d =  new Date(date),
                ...
                day = this.dayTabel(config,options),
         ...
         * @returns {string}
         */
        dayTabel : function(config,options){
            var year = options.year,
                ...
                next = ChCalendar.util.calculationMonth(year + "/" + month),//下一月日期
    ...
    Operation = {
        ...
        setCurrentPanel : function(example,options){
            ...
            if(!example.domConfig.currentDate){
                example.domConfig.currentDate = example.o.currentDate || example.o.date.start;
            }
            example.domConfig.currentDate = ChCalendar.util.formatDate(example.domConfig.currentDate,meConfig.format)
        ...
        eventBind : function(example,myThis){
                ...
                if(config.type === "day"){//日历事件
                    $(example.$target).on('click.selectDay','[target-type="day"]',function(){//日期选择事件
                        ...
                        if(!$this.hasClass("nextDay") && !$this.hasClass("preDay")){
                            ...
                            example.domConfig.currentDate = date;//永远显示当前点击日期的所属月
                            _this.reloadCalendar(example);
                        }
            ...
            if(config.type === "day"){
                $(example.$target).on('click.selectMonth','[target-type="month"]',function(){//月选择事件
                    ...
                    example.domConfig.currentDate = date;
                    example.domConfig.renderType = 'day';
                    _this.reloadCalendar(example);
            ...
            $(example.$target).on('click.selectYear','[target-type="year"]',function(){//年选择
                var date =  $(this).data("type");
                example.domConfig.currentDate = date +"/"+ (new Date(example.domConfig.currentDate).getMonth() + 1);
                example.domConfig.renderType = 'month';
                _this.reloadCalendar(example);
            ...
            $(example.$target).on('click.header','.Ch_Calendar_header i,.Ch_Calendar_header b',function(e){
                var $target = $(e.target),
                    ...
                    renderStatus = true,
                    msg = {
                        type : example.domConfig.renderType
                    };
                switch (type){
                    case 'pre':
                        _this.pre(example);
                        msg.currentDate = example.domConfig.currentDate;
                    ...
                    case 'next':
                        ...
                        break;
                    default:
                        switch (example.domConfig.renderType){
                            case "day":
                                example.domConfig.renderType = "month";
                                break;
                            case "month":
                                example.domConfig.renderType = "year";
                                break;
                            case "year":
                                break;
                        }
        ...
        testDateDisable : function(options,date){
            ...
            if(options) {
                if(date.constructor !== Array) {
                    if (options.last) {
                        max = ChCalendar.util.testMacthMax(date, options.last);
                    }
                    if (options.first) {
                        min = ChCalendar.util.testMacthMin(date, options.first);
                    }
        ...
        next : function(example){
            ...
            switch (domConfig.renderType){
                case "day":
                    date = ChCalendar.util.formatDate(ChCalendar.util.calculationMonth(domConfig.currentDate,+1),meConfig.format);
                    break;
                case "month":
                    var y = parseInt(domConfig.currentDate.substring(0,4)) + 1,
                        o = domConfig.currentDate.substring(4,10);
                    date = y + o;
                    break;
                case "year":
                    var y = parseInt(domConfig.currentDate.substring(0,4)) + 12,
                        o = domConfig.currentDate.substring(4,10);
                    date = y + o;
                    break;
                default :
            ...
            this.setCurrentPanel(example,{currentDate : date});
